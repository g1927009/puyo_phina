...
//    this.movablePuyoElement.style.top = y + 'px';
  ...
  falling (isDownPressed) {
    ...
    if(!isBlocked) {
      ...
      this.puyoStatus.top = this.puyoStatus.top + (CONFIG_PLAYER_FALLING_SPEED + (this._score / 5000));
      if(isDownPressed) {
        // 下キーが押されているならもっと加速する
        this.puyoStatus.top = this.puyoStatus.top + (CONFIG_PLAYER_DOWN_SPEED + (this._score / 5000));
      }
